...
> 
> 🛑 **【AI 与开发者必读警告】** 🛑
> **任何人（包括 AI 助手）在修改本项目代码之前，必须完整阅读并理解 `docs/` 目录下的所有文档（尤其是 `prompt-conventions.md` 中定义的极其严苛的系统规范与防伪命名铁律）！不要靠猜测去破坏已经跑通的核心寻图与存储逻辑！**
...
4. **提示词可视化管理中心 (Prompt Studio)**：告别硬编码！在独立的可视化界面中全面接管 11 个核心编导参数，支持动态变量 `{{占位符}}` 注入，一次修改全局生效。
5. **一站式文件管理**：项目制管理，音频、视频、图片、脚本全部自动归档，告别“桌面垃圾堆”。
