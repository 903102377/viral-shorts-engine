...
interface ProjectContextValue {
  ...
  sceneVisualPromptsZh: Record<number, string>;
  setSceneVisualPromptsZh: React.Dispatch<React.SetStateAction<Record<number, string>>>;
  sceneCameraPrompts: Record<number, string>;
  setSceneCameraPrompts: React.Dispatch<React.SetStateAction<Record<number, string>>>;
  sceneCameraPromptsZh: Record<number, string>;
  setSceneCameraPromptsZh: React.Dispatch<React.SetStateAction<Record<number, string>>>;
...
export function ProjectProvider({ children, projectId }: { children: React.ReactNode; projectId: string }) {
  ...
  const [sceneVisualPrompts, setSceneVisualPrompts] = useState<Record<number, string>>({});
  const [sceneVisualPromptsZh, setSceneVisualPromptsZh] = useState<Record<number, string>>({});
  const [sceneCameraPrompts, setSceneCameraPrompts] = useState<Record<number, string>>({});
  const [sceneCameraPromptsZh, setSceneCameraPromptsZh] = useState<Record<number, string>>({});
  ...
  useEffect(() => {
    fetch(`/api/state?projectId=${encodeURIComponent(projectId)}`).then(r => r.json()).then(res => {
      if (res.success && Object.keys(res.data).length > 0) {
        ...
        if (data.sceneVisualPrompts) setSceneVisualPrompts(data.sceneVisualPrompts);
        if (data.sceneVisualPromptsZh) setSceneVisualPromptsZh(data.sceneVisualPromptsZh);
        if (data.sceneCameraPrompts) setSceneCameraPrompts(data.sceneCameraPrompts);
        if (data.sceneCameraPromptsZh) setSceneCameraPromptsZh(data.sceneCameraPromptsZh);
  ...
  useEffect(() => {
    ...
    const data = {
      projectId, theme, flowUrl, artStyle, characters, scriptLines, currentPhase,
      // v2 新增
      publishInfo,
      ...
      sceneVisualPrompts, sceneVisualPromptsZh, sceneCameraPrompts, sceneCameraPromptsZh, sceneCharacters,
      sceneDurations, sceneVideoTrimStart, sceneVideoTrimEnd, sceneImages, sceneImageRefs, sceneVideos, sceneAudio, sceneAudioDelays,
  ...
  const handleBrainstormDirectly = useCallback(async (themeParam: string) => {
    ...
    try {
      const promptData = await fetchApi('/api/generate-prompts', { taskType: 'script', theme: themeParam });
  ...
  }, [theme, creativeMode, userDirection]);
  ...
  }, [characters, artStyle, getFullScriptContext]);
  ...
  const generateCastingImage = useCallback(async (index: number) => {
    ...
    try {
      await fetch('/api/extension/active-context', { method: 'POST', body: JSON.stringify({ projectId, targetType: 'characterImage', index }) });
  ...
  }, [characterPrompts, flowUrl, projectId, useHitlMode]);
  ...
  const handleGenerateActionPrompt = useCallback(async (i: number) => {
    ...
    try {
      ...
      const allCharactersContext = characters.map(c => `${c.name}: ${c.persona}`).join('\n');
      const previousVisualPrompt = i > 0 ? sceneVisualPrompts[i - 1] : "";
      const previousCameraPrompt = i > 0 ? sceneCameraPrompts[i - 1] : "";
      const data = await fetchApi('/api/generate-prompts', {
          taskType: 'action',
          artStyle,
          fullScriptContext: getFullScriptContext(),
          ...
          previousCameraPrompt
      });
      setSceneVisualPrompts(p => ({ ...p, [i]: data.visual_prompt }));
      if (data.visual_prompt_zh) setSceneVisualPromptsZh(p => ({ ...p, [i]: data.visual_prompt_zh }));
      setSceneCameraPrompts(p => ({ ...p, [i]: data.camera_prompt }));
      if (data.camera_prompt_zh) setSceneCameraPromptsZh(p => ({ ...p, [i]: data.camera_prompt_zh }));
  ...
  }, [scriptLines, characters, artStyle, getFullScriptContext]);
  ...
  }, [publishInfo, artStyle, getFullScriptContext]);
  ...
  const handleGenerateStartFrame = useCallback(async (i: number) => {
    ...
    try {
      const prompt = sceneVisualPrompts[i];
      ...
      // 如果 prompt 里包含 {@场景}，将其塞入 refKeywords
      ...
      alert('生成首图失败: ' + e.message);
    } finally {
      setProcessingScene(p => ({ ...p, [i]: null }));
    }
  }, [sceneVisualPrompts, sceneCharacters, characters, characterPrompts, scriptLines, flowUrl, projectId, useHitlMode]);

  const handleGenerateVideo = useCallback(async (i: number) => {
    setProcessingScene(p => ({ ...p, [i]: 'video' }));
    try {
      const visualPrompt = sceneVisualPrompts[i];
      const cameraPrompt = sceneCameraPrompts[i] || "";
      const startImage = sceneImages[i];
      if (!visualPrompt || !startImage) throw new Error("请确保有视觉构图词和首图！");

      // ====== Veo 3.1 完整体验 ======
      // 此时我们不再手动肢解 Audio 内容了。直接原样将包含中文对白和运镜词的 Prompt 一并发送给打通的代理。
      let prompt = cameraPrompt.trim();
      
      // 3. 极其关键的回调：向 Playwright 指定，要去素材库里找【刚才 Nano 生成的那张首图】当底片！
      // 因为首图是用 visualPrompt 生成的，所以搜索 visualPrompt 就能精准定位到那张首图。
      // 但对于 HITL 模式，如果是插件传进来的，这里的 referenceKeyword 会变成如 "VS_Scene_3" 的精准 ID。
      const finalReferenceKeyword = sceneImageRefs[i] || visualPrompt;

      await fetch('/api/extension/active-context', { method: 'POST', body: JSON.stringify({ projectId, targetType: 'sceneVideo', index: i }) });
      const data = await fetchApi('/api/generate-assets', { prompt, model: 'Veo 3.1', referenceKeywords: [finalReferenceKeyword], flowUrl, projectId, fireAndForget: useHitlMode, veoMode: 'frame' });
  ...
  }, [sceneVisualPrompts, sceneCameraPrompts, sceneImages, sceneImageRefs, flowUrl, projectId, useHitlMode]);
  ...
  const handleClearProgress = useCallback(() => {
    ...
    setSceneVisualPrompts({});
    setSceneVisualPromptsZh({});
    setSceneCameraPrompts({});
    setSceneCameraPromptsZh({});
    setSceneCharacters({});
    setSceneDurations({});
    setSceneImages({});
    setSceneImageRefs({});
  ...
  const value: ProjectContextValue = {
    ...
    processingScene,
    handleGenerateActionPrompt, handleGenerateStartFrame,
